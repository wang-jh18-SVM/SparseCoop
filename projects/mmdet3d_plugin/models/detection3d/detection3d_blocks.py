...
class SparseBox3DEncoder(BaseModule):
    ...
    def forward(self, box_3d: torch.Tensor):
        ...
        size_feat = self.size_fc(box_3d[..., [L, W, H]])
    ...
    def __init__(
        ...
        if with_cls_branch:
            self.cls_layers = nn.Sequential(
                *linear_relu_ln(embed_dims, 1, 2), Linear(self.embed_dims, self.num_cls)
            )
        self.with_quality_estimation = with_quality_estimation
        if with_quality_estimation:
            self.quality_layers = nn.Sequential(
                *linear_relu_ln(embed_dims, 1, 2), Linear(self.embed_dims, 2)
            )
    ...
    def anchor_projection(
        ...
        for i in range(len(T_src2dst_list)):
            ...
            T_src2dst = torch.unsqueeze(
                T_src2dst_list[i].to(dtype=anchor.dtype), dim=-3
            )
            ...
            size = anchor[..., [L, W, H]]
            yaw_cos_sin = torch.matmul(
                T_src2dst[..., :2, :2], anchor[..., [COS_YAW, SIN_YAW], None]
            ).squeeze(-1)
            yaw_sin_cos = yaw_cos_sin[..., [1, 0]]
            vel = torch.matmul(
                T_src2dst[..., :vel_dim, :vel_dim], vel[..., None]
            ).squeeze(-1)
            dst_anchor = torch.cat([center, size, yaw_sin_cos, vel], dim=-1)
