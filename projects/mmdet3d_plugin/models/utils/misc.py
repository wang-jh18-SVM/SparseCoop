...
def locations(features, stride, pad_h, pad_w):
    """Generate normalized coordinate maps for feature maps.

    This function creates a grid of (x, y) coordinates corresponding to the spatial locations in the feature map, normalized by the padded image dimensions.

    Args:
        features (torch.Tensor): Feature map tensor of shape (N, C, H, W)
        stride (int): Stride value that determines the spacing between grid points
        pad_h (int): Padded height of the original image
        pad_w (int): Padded width of the original image

    Returns:
        torch.Tensor: Coordinate map of shape (H, W, 2), where each point contains normalized (x, y) coordinates in range [0,1]. The coordinates correspond to the center points of each cell in the feature grid.
    """
    # Extract height and width of the feature map
    h, w = features.size()[-2:]
    device = features.device

    # Generate x-coordinates with stride spacing, centered in each cell (+ stride//2) and normalized by padded width
    shifts_x = (
        torch.arange(0, stride * w, step=stride, dtype=torch.float32, device=device)
        + stride // 2
    ) / pad_w

    # Generate y-coordinates with stride spacing, centered in each cell (+ stride//2) and normalized by padded height
    shifts_y = (
    ...
    # Stack x and y coordinates to get (x,y) pairs
    locations = torch.stack((shift_x, shift_y), dim=1)

    # Reshape to match the feature map spatial dimensions (H, W, 2)
