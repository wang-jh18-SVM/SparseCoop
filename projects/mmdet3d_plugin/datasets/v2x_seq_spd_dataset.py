...
class V2XSeqSPDDataset(Dataset):
    DefaultAttribute = {
        "car": "vehicle.parked",
    ...
    def __init__(
        ...
        self.last_id = None
        if with_seq_flag:
            self.seq_time_threshold = seq_time_threshold
        ...
        if self.v2x_side == "infrastructure-side":
            # Modify class ranges to 2x
            nusc_det_config = self.det3d_eval_configs.serialize()
            for key in nusc_det_config['class_range'].copy():
                if key in self.CLASSES:
                    self.det3d_eval_configs.class_range[key] *= 2
                else:
                    self.det3d_eval_configs.class_range.pop(key)
            nusc_track_config = self.track3d_eval_configs.serialize()
            for key in nusc_track_config['class_range'].copy():
                if key in self.CLASSES:
                    self.track3d_eval_configs.class_range[key] *= 2
                else:
                    self.track3d_eval_configs.class_range.pop(key)
    ...
    def _set_sequence_group_flag(self):
        ...
        for idx in range(len(self.data_infos)):
            # In V2X-Seq-SPD dataset, a new sequence begins when:
            # 1. It's not the first frame (idx != 0)
            # 2. It has a different scene_token or its timestamp is much greater than the previous frame
            if idx != 0 and (
                (
                ...
                or (
                    self.data_infos[idx]["timestamp"] / 1e6
                    - self.data_infos[idx - 1]["timestamp"] / 1e6
                    > self.seq_time_threshold
                )
                or (
                    self.data_infos[idx]["timestamp"] / 1e6
                    - self.data_infos[idx - 1]["timestamp"] / 1e6
    ...
    def get_data_info(self, index):
        ...
        if self.v2x_side == "cooperative" or self.v2x_side == "early-fusion":
            # # For CVPR challenge, load pkl in UniV2X Format
            # inf_info = info['other_agent_info_dict']['model_other_agent_inf']
            # sample_idx_inf = inf_info['token']
            # veh2inf_r = inf_info['VehLidar2InfLidar_rotation']
            # veh2inf_t = inf_info['VehLidar2InfLidar_translation']

            # Usual case
            sample_idx_inf = info['token_inf']
            ...
            input_dict.update(
                dict(
                    veh2inf_rt=veh2inf_rt.astype(np.float32),
                    sample_idx_inf=sample_idx_inf,
                )
            )
        ...
        if self.modality["use_camera"]:
            ...
            for cam_type, cam_info in info["cams"].items():
                ...
                lidar2cam_r = cam_info["lidar2cam_rotation"]
                lidar2cam_t = cam_info["lidar2cam_translation"][:, 0]
            ...
            # # For CVPR challenge, load pkl in UniV2X Format
            # if self.v2x_side == "early-fusion":
            #     for cam_type, cam_info in inf_info["cams"].items():
            #         image_paths.append(
            ...
            #         lidar2img_rts.append(lidar2img_rt @ veh2inf_rt)
            #         cam_intrinsics.append(viewpad)
            #         lidar2cam_rts.append(lidar2cam_rt @ veh2inf_rt)
    ...
    def get_ann_info(self, index):
        ...
        if self.v2x_side == "cooperative":
            instance_id_veh = info["gt_veh_track_ids"][mask].astype(np.int64)
            instance_id_inf = info["gt_inf_track_ids"][mask].astype(np.int64)
            anns_results.update(
                dict(
                    instance_id_veh=instance_id_veh,
                    instance_id_inf=instance_id_inf,
                )
            )
            ...
            for i, box in enumerate(boxes):
                ...
                if np.sqrt(box.velocity[0] ** 2 + box.velocity[1] ** 2) > 0.2:
                    if name in [
                        ...
                        attr = V2XSeqSPDDataset.DefaultAttribute[name]
                else:
