    ...
    for sample_token in tqdm.tqdm(sample_tokens, leave=verbose):
        ...
        for sample_annotation_token in sample_annotation_tokens:
            ...
            if box_cls == DetectionBox_modified:
                ...
                detection_name = category_to_type_name(
                ...
                tracking_name = category_to_type_name(
                    sample_annotation['category_name']
                )
...
class TrackingEval_custom(TrackingEval):
    ...
    def __init__(
        ...
        # When pred_boxes are less, filter the gt_boxes
        if len(pred_boxes.sample_tokens) < len(gt_boxes.sample_tokens) and set(
            pred_boxes.sample_tokens
        ).issubset(set(gt_boxes.sample_tokens)):
            print(
                f"Warning: Pred boxes are subset of gt boxes, filtering gt boxes. {len(pred_boxes.sample_tokens)} pred boxes, {len(gt_boxes.sample_tokens)} gt boxes"
            )
            gt_boxes = filter_by_sample_token(gt_boxes, pred_boxes.sample_tokens)
...
class NuScenesEval_custom(NuScenesEval):
    ...
    def __init__(
        ...
        # When pred_boxes are less, filter the gt_boxes
        if len(self.pred_boxes.sample_tokens) < len(
            self.gt_boxes.sample_tokens
        ) and set(self.pred_boxes.sample_tokens).issubset(
