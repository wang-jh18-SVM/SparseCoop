...
from .utils import draw_lidar_bbox3d_on_img, draw_lidar_bbox3d_on_bev
...
class NuScenes3DDetTrackDataset(Dataset):
    ...
    def _set_sequence_group_flag(self):
        ...
        for idx in range(len(self.data_infos)):
            # In NuScenes dataset, a new sequence begins when:
            # 1. It's not the first frame (idx != 0)
            # 2. The current frame has no sweeps (previous frames data)
    ...
    def show(self, results, save_dir=None, show=False, pipeline=None):
        ...
        for i, result in enumerate(results):
            ...
            bev = draw_lidar_bbox3d_on_bev(
                pred_bboxes_3d, bev_size=img.shape[0] * 2, color=color
            )
            ...
            for j, name in enumerate(
                ...
                imgs[j] = cv2.rectangle(
                    imgs[j], (0, 0), (440, 80), color=(255, 255, 255), thickness=-1
                )
