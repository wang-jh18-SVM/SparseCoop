...
num_cams = 1
train_sample_num = 1495  # * different for every dataset
class_names = [
    "car",
    # "truck",
...
log_config = dict(
    ...
    hooks=[
        ...
        dict(
            ...
            init_kwargs=dict(
                project='sparsecoop',
                name=f'v1-4_{dataset_prefix}_{v2x_side}_lr{lr}_bs{batch_size_per_gpu}x{num_gpus}_{num_epochs}e_153CT_woAnchorCoop',
            ),
        ),
    ],
)
load_from = "work_dirs/V2X-Seq-SPD-2hz/vehicle-side/sparsecoop_v1-1_r50_544x960_bs6x8_800e/iter_23200.pth"
resume_from = None
workflow = [("train", 1)]
...
read_track_query_file_root = (
    f'./data/infos/{dataset_prefix}/infrastructure-side/track_query_dn/'
)
inf_keys = [
...
model = dict(
    ...
    head=dict(
        ...
        instance_bank=dict(
            ...
            embed_dims=embed_dims,
            anchor=f"data/infos/{dataset_prefix}/vehicle-side/spd_kmeans900.npy",
            ...
            cross_agent_interaction=dict(
                ...
                coop_vis_image_bound=True,
                coop_vis_dist_threshold=-1,
                id_mapping_config={  # V2X-Seq-SPD-2hz configuration: 2Hz, 20~30s scenes
                    'data_frequency': 2.0,
                    'max_entries': 2000,
                    'stale_threshold': 5.0,
                    'expire_threshold': 30.0,  # slightly longer than scene duration
                    'confidence_threshold': 0.1,
                    'cleanup_interval': 30,  # Every 15 seconds
                },
                # Options for matching
                match_veh_conf_threshold=0.02,
                match_dim_threshold=0.7,
                match_feat_weight=0.0,
                box_weights=[1.0] * 3 + [0.0] * 8,
            ),
            v2x_side=v2x_side,
            only_pos_dn=True,
            coop_dn_noise_trans_std=1.0,
            coop_dn_noise_rot_std=2.0,
            coop_anchor_wo_latency_compensation=True,
        ),
        anchor_encoder=dict(
...
data_root = f"datasets/{dataset_prefix}/{v2x_side}/"
info_root = f"./data/infos/{dataset_prefix}/{v2x_side}/"
ann_file_train = info_root + "modify_infos_train_filter_3d_gt.pkl"
ann_file_val = info_root + "modify_infos_val_filter_3d_gt.pkl"
