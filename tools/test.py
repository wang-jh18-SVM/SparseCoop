...
def parse_args():
    ...
    parser.add_argument(
        "--out",
        action='store_true',
        help="whether to output result file in pickle format",
    )
    parser.add_argument(
        "--fuse-conv-bn",
        action="store_true",
...
def main():
    ...
    if rank == 0:
        ...
        if checkpoint_dir.startswith('work_dirs/'):
            work_dir = checkpoint_dir
        else:
            work_dir = args.config.replace('projects/configs/', 'work_dirs/').replace(
                '.py', ''
            )
        ...
        if args.out:
            print(f"\nwriting results to {osp.join(val_dir, 'results.pkl')}")
            mmcv.dump(outputs, osp.join(val_dir, 'results.pkl'))

        kwargs = {} if args.eval_options is None else args.eval_options
        kwargs['jsonfile_prefix'] = val_dir
        if args.show_only:
