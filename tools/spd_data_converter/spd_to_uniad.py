...
class Box3D():
    def __init__(self):
        ...
        self.prev = None
        self.next = None
        # Add cooperative tracking fields
        self.veh_track_id = None
        self.inf_track_id = None
        self.veh_token = None
        self.inf_token = None
...
def create_spd_infos_coop(root_path,
    ...
    for coop_data_info in tqdm(coop_data_infos):        
        ...
        for anno_token in annotations.keys():
            ...
            # Add cooperative tracking fields
            box3d.veh_track_id = annotation.get('veh_track_id', '')
            box3d.inf_track_id = annotation.get('inf_track_id', '')
            box3d.veh_token = annotation.get('veh_token', '')
            box3d.inf_token = annotation.get('inf_token', '')
        ...
        # Extract cooperative tracking fields
        veh_track_ids = np.array([b.veh_track_id for b in boxes])
        inf_track_ids = np.array([b.inf_track_id for b in boxes])
        veh_tokens = np.array([b.veh_token for b in boxes])
        inf_tokens = np.array([b.inf_token for b in boxes])
        ...
        # Add cooperative tracking fields to info
        info['gt_veh_track_ids'] = veh_track_ids
        info['gt_inf_track_ids'] = inf_track_ids
        info['gt_veh_tokens'] = veh_tokens
        info['gt_inf_tokens'] = inf_tokens
        ...
        for ii in range(len(cur_instance_samples) - 1):
            ...
            while cur_frame_idx != cur_instance_samples[ii + 1]['frame_idx']:
                ...
                cur_instance_sample_anno = {
                    ...
                    "truncated_state": 0,
                    "occluded_state": 3,
                    "3d_dimensions": cur_instance_samples[ii]['annotation'][
                        '3d_dimensions'
                    ],
                    "3d_location": cur_loc,
                    "rotation": cur_rot,
                    "instance_token": instance_token,
                    # Add cooperative tracking fields for interpolated annotations
                    "veh_track_id": cur_instance_samples[ii]['annotation'].get(
                        'veh_track_id', ''
                    ),
                    "inf_track_id": cur_instance_samples[ii]['annotation'].get(
                        'inf_track_id', ''
                    ),
                    "veh_token": cur_instance_samples[ii]['annotation'].get(
                        'veh_token', ''
                    ),
                    "inf_token": cur_instance_samples[ii]['annotation'].get(
                        'inf_token', ''
                    ),
                }
