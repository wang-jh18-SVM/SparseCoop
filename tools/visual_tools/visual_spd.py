...
def visualize_sample(
    ...
    pc, _ = CustomLidarPointCloud.from_file_multisweep(
...
def get_sample_data(
    ...
    # hardcode for camera calib token
    if side in ['vehicle-side', 'cooperative']:
        cs_record = nusc.get(
            'calibrated_sensor', 'a4debdb5-22b2-3269-b7d7-756f1d560c92'
        )
        ...
        cs_record = nusc.get(
            'calibrated_sensor', '23ef3a7f-ebdc-389f-a831-34b76331632a'
        )
        ...
        cs_record = nusc.get(
            'calibrated_sensor', '09e0b220-9e47-3742-9b39-848af4145538'
        )
...
def lidar_render(
    sample_token, data, ax=None, out_path=None, side='vehicle-side', thre=0.0, nusc=None
):
    bbox_gt_list = []
    bbox_pred_list = []
...
def render_sample_data(
    ...
    boxes_pred = [
        Box(
            ...
            name=record['tracking_name'],
            token=record['tracking_id'],
        )
        for record in pred_data['results'][sample_token]
        if record['tracking_score'] > thre
    ]
    boxes_gt = nusc.get_boxes(sample_data_token)
    data_path, boxes_pred, camera_intrinsic = get_sample_data(
        sample_data_token,
        box_vis_level=box_vis_level,
        boxes=boxes_pred,
        side=side,
...
def to_video(folder_path, out_path, fps=4, downsample=1):
    """
    Create a video from a sequence of visualization images at full resolution.
    ...
    Args:
        ...
        downsample: Downsample factor (ignored - always uses full resolution)
    """
    imgs_path = glob.glob(os.path.join(folder_path, '*.png'))
    imgs_path = sorted(imgs_path)
    img_array = []
    for img_path in tqdm(imgs_path, desc="Reading images"):
    ...
    if img_array:
        # Use original image size for full resolution
        size = (img_array[0].shape[1], img_array[0].shape[0])
        # Use MP4 codec for better compression and compatibility
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--predroot',
        default='work_dirs/V2X-Seq-SPD-2hz/cooperative/sparsecoop_v1-2_r50_544x960_bs4x8_100e_no_aug_co153CT/test/Mon_Jul__7_18_48_04_2025/img_bbox/results_nusc_tracking.json',
        help='Path to json',
    )
    parser.add_argument('--side', default='cooperative', help='side')
    parser.add_argument('--is_gt', default=False, help='plot gt')
    parser.add_argument(
        '--dataroot',
        default='datasets/V2X-Seq-SPD-2hz-for-SparseCoop/',
        help='path to data',
    )
    ...
    side = args.side
    root_path = args.predroot.replace('results_nusc_tracking.json', 'result_vis')
    ...
    video_path = os.path.join(root_path, side + '.mp4')
